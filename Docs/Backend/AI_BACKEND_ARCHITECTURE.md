...
- `user_id`는 두지 않습니다.
- `.env`의 GameClient/WebClient 개발 Token을 분리합니다.
- Backend가 Token에서 `profile_id`, `device_id`, Device 역할을 결정합니다.
- Client Body의 동일 ID는 선택적 주장값이며 인증 결과와 다르면 거부합니다.
- 개발 Token 원문은 Commit, Fixture, URL과 로그에 포함하지 않습니다.
- M02부터 개발 인증기를 Pairing 발급 Token과 Hash 기반 저장소로 교체합니다.
...
- 장기 Token 원문은 DB에 저장하지 않고 Hash를 저장합니다.
